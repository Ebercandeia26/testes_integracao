...
public class CadastroProdutosTest {  
    ...
    public void testAtualizarProdutoExistente() {  
        Produto novoP1 = new Produto(1, "Arroz Integral", 12.0, 75);  
        cadastro.atualizarProduto(novoP1);  
        verify(p1).setNome("Arroz Integral");  
        verify(p1).setPreco(12.0);  
        verify(p1).setQuantidade(75);  
        verifyNoMoreInteractions(p1);  
        List<Produto> produtos = cadastro.getProdutos();  
        assertEquals(Arrays.asList(novoP1, p2), produtos);  
    }  
